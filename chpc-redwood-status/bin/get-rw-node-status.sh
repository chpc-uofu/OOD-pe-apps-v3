...
readonly working_dir="/uufs/chpc.utah.edu/sys/ondemand/chpc-pe-apps/chpc-redwood-status"
cd "$working_dir"

readonly density_limit=10 # save one per ten minutes
readonly snapshot_dir="/uufs/chpc.utah.edu/sys/ondemand/node-usage-snapshots/snapshot-rw"
...
paste                                   \
  <(/uufs/redwood.bridges/sys/installdir/slurm/std/bin/sinfo -M redwood --Node -o "$sinfo_short_fmt" | grep -v CLUSTER | sed -e s/\"/\\\\\"/g -e s/\'/\"/g ) \
  <(/uufs/redwood.bridges/sys/installdir/slurm/std/bin/sinfo -M redwood  -M redwood --Node -O "$sinfo_long_fmt" | grep -v CLUSTER  ) \
  > "$snapshot_dat"
./bin/plot-rw-node-status.py "$snapshot_dat" "$snapshot_job_stats_dat"
